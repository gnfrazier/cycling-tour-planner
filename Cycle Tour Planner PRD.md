...
* Wants to access and maintain rider profiles of their clients
...
- \[ ] Route uses local elevation data as the primary source, falling back to OSMNx google based API only when data is sparse/unavalible. 
...
|FR30|System suggests routes/segments weighted toward frequently-cycled bike routes, greenways, and rail trails|TBD|Candidate plug-in based feature — data source (OSM cycle-infrastructure tags vs. a heatmap-style service) and scoring approach undecided; see Open Questions|
|FR31|System incorporates route/segment popularity data (Strava-heatmap-style signal) into route suggestions|TBD|Candidate plug-in based feature — no OSM-native equivalent; would require a new third-party data source, in tension with the open-data/local-first bias (§4.4); see Open Questions|
...
|FR34|User sets the route's start point — and optional destination and intermediate waypoints — via location search (geocoding) for Online status, a tap on the mapfor offline status, or the device's current GPS location for offline status. This is the entry step before any theme routing runs|P0|Requires a geocoding service (e.g. Nominatim/Photon over OSM data), a new external dependency subject to the responsible-use caching discipline (§4.4); offline behavior on Mobile or desktop relies on the trip's bundled extract for in-bounds search — see Open Questions #20–21|

...
|Streetview-style imagery during planning — candidate premium-tier feature (proposed, undecided; see §3.6)|FR27|P2|
|Desktop-only Street View hyperlapse preview via streetwarp-cli — candidate premium-tier feature (proposed, undecided; see §3.6)|FR28|TBD|
...
|Premium "trip version history" — automatically retain the last 5–10 versions of a trip for rollback|FR33|TBD (future, post-M9)|
...
* Payment/monetization of any kind (see §3.6 for a proposed premium-tier concept that would conflict with this — flagged, not resolved)
...
* **Premium framing (proposed, undecided)**: This hyperlapse phase and FR27 (general streetview-style imagery) are candidate premium-tier features, aligned with the proposed Premium User concept in §3.6 — both are already login-gated, and the metered, paid Street View Static API dependency makes premium-gating a natural cost control on top of that. This is a proposed direction, not a commitment — see §3.6 for the same monetization tension flagged against §3.3
* **Related**: FR27 (general streetview-style imagery during planning, not a hyperlapse) is scheduled adjacent to this phase (M9) since it likely shares this section's Street View Static API dependency, billing concern, and now premium framing — see §7

### 3.6 Future Phase — Premium Trip Version History (TBD)

A proposed premium tier, scheduled after all other milestones including the streetview/hyperlapse phase (see M10 in §8, after M9) — later than §3.5's already-last phase, since it isn't committed at all yet.
...
* Cannot view streetview-style imagery/hyperlapse (candidate premium features, proposed/undecided — see §3.5, §3.6) or save a trip to an account — both sit behind a login/account boundary, and streetview specifically may sit behind a premium boundary beyond that if the proposed premium tier is ever adopted
...
- \[ ] Streetview imagery/hyperlapse and saving a trip to an account prompt a login (and, if the proposed premium tier is ever adopted, a premium upsell for streetview specifically) rather than silently failing or being hidden
...
|FR27|User can view streetview-style or other point-of-view imagery during planning|P2|Candidate premium-tier feature (proposed, undecided — see §3.5, §3.6); may share the paid Street View Static API dependency with FR28 (§3.5) — see risk in §7|
|FR28|System can generate (or hand off to) a desktop-only Street View hyperlapse preview of a route via streetwarp-cli integration|TBD|Candidate premium-tier feature (proposed, undecided — see §3.5, §3.6); desktop-only, absent on mobile/web — that absence is explained in-product at the handoff, not left to silently vanish (§4.4/§6 quiet-clarity stance); see §3.5 — implementation and interaction intentionally undecided|
...
|M9 — Streetview imagery \& hyperlapse|Streetview-style imagery during planning (FR27) plus desktop-only streetwarp-cli hyperlapse integration (FR28, TBD) — both candidate premium-tier features (proposed, undecided; §3.5, §3.6)|FR27 grouped here since it likely shares FR28's Street View Static API dependency (§7) and now its premium framing; hyperlapse implementation deliberately undetermined until this phase starts (§3.5)|
|M10 — Premium trip version history (TBD, not committed)|Proposed premium tier automatically retaining the last 5–10 versions of a trip for rollback (FR33)|Nothing yet — proposed future scope only, scheduled after every other milestone (§3.6); conflicts with the no-monetization stance in §3.3, flagged not resolved|
